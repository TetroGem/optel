...
export type AssertAssignableTo<T, S> = S extends Partial<Record<infer U, any>>[] ? ([
    Extract<U, ReadonlyKey<T>>
] extends [never] ? S : "Sources cannot overwrite readonly properties of target"[]) : "Source types are not assignable to target type"[];
